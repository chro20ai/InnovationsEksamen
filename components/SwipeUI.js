      ...
      this.PanResponder = PanResponder.create({
        ...
        onPanResponderRelease: (evt, gestureState) => {
          ...
          if (gestureState.dx > 120) {
            ...
            //Vi smider indekset ind i et array, så vi kan holde styr på hvad der er liket
...
// bruger absolute position så billederne stacker, stedet for at være over og under hinanden 
  ...
  let scorearray = []
  let idarray = []
  //Vi looper igennem recipearray og får opskrifterne
  //Så tager vi ingredientarray som er det brugeren skriver ind, og tager hver ingredienst.
  //Nu tager vi hver recipe i recipearray og tager hver ingrediens for hver recipe
  //Vi undersøger om der er match, hvis ja, så lægges en til scoreren
  recipearray.forEach(recipe => {
  ingredientarray.forEach(element => {
      recipe.forEach(ingredient => {if(ingredient == element){score++}})
    })
    //Vi smider scoreren ind i et array for scorerne
    scorearray.push(score)
    //Vi smider opskriftens id ind i et array
    idarray.push(recipe[0])
    //Scoreren nulstilles for efter en recipe  
    score = 0
  })
  //Nu kan vi så tage den med højest score og finde indekset
  scorearray.indexOf(Math.max.apply(Math, scorearray))

let finalarray = []
//Vi looper igennem scorearray indtil det er tomt og finder indekset på den med den højeste værdi
 for ( let i = 0 ; scorearray.length !== 0 ; i++ ){
  let maxindex = scorearray.indexOf(Math.max.apply(Math, scorearray))
  
  //Vi smider det id med højest score over i endnu et array, så dem med højest score vises først
  finalarray.push(idarray[maxindex])

  //Vi fjerner ny elementet som lige håndteret i både scorearray og idarray, så de ikke skubbes ind flere gange
...
let newuserarray = []
//Looper gennem finalarray og skubber billederne over, så det er dem der vises i den rigtige rækkefølge
